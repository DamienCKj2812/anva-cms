...
class AttributeService extends BaseService {
  ...
  private async createPrimitiveAttributeValidation(
    ...
    if (enumValues !== undefined) {
      this.validateEnumValue(enumValues, attributeType, attributeFormat, repeatable, validation, defaultValue);
    }
  ...
  private async updatePrimitiveAttributeValidation(attribute: Attribute, data: UpdatePrimitiveAttributeDTO): Promise<UpdatePrimitiveAttributeDTO> {
    ...
    if (enumValues !== undefined) {
      ...
      this.validateEnumValue(enumValues, attributeType || attribute.attributeType, attributeFormat, repeatable, validation, defaultValue);
    }
  ...
  async getOrderMap(contentCollection: ContentCollection): Promise<Record<string, string[]>> {
    ...
    orderMap["root"] = attributes.map((attr) => attr._id.toString());

    const componentIdsSet = new Set<string>();
    for (const attr of attributes) {
      if (attr.componentRefId) {
        componentIdsSet.add(attr.componentRefId.toString());
      }
    }

    if (componentIdsSet.size > 0) {
      const componentAttributes = await this.collection
        .find({
          componentRefId: { $in: Array.from(componentIdsSet).map((id) => new ObjectId(id)) },
          attributeKind: AttributeKindEnum.COMPONENT_PRIMITIVE,
        })
        .sort({ position: 1 })
        .toArray();

      for (const componentId of componentIdsSet) {
        orderMap[componentId] = componentAttributes
          .filter((attr) => attr.componentRefId?.toString() === componentId)
          .map((attr) => attr._id.toString());
      }
    }
  ...
  public validateEnumValue(
    ...
    if (attributeType !== AttributeTypeEnum.STRING) {
      throw new ValidationError("enumValues are only supported for string attributes");
    }
    if (attributeFormat !== undefined) {
      throw new ValidationError("enumValues cannot have attributeFormat");
    }
    if (repeatable === true) {
      throw new ValidationError("enumValues cannot be repeatable");
    }
    if (validation !== undefined) {
      throw new ValidationError("enumValues cannot have validation");
    }
    if (defaultValue !== undefined) {
      throw new ValidationError("enumValues cannot have defaultValue");
    }
  }
